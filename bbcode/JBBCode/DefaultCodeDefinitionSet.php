...
class DefaultCodeDefinitionSet implements CodeDefinitionSet
{
    ...
    public function __construct()
    {
        ...
        /* [url] link tag */
        $builder = new CodeDefinitionBuilder('url', '<a href="{param}" target="_blank" class="text-link">{param}</a>');
        $builder->setParseContent(false)->setBodyValidator($urlValidator);
        array_push($this->definitions, $builder->build());

        /* [url=http://example.com] link tag */
        $builder = new CodeDefinitionBuilder('url', '<a href="{option}" target="_blank" class="text-link">{param}</a>');
